...
### 4. denoising step을 늘려도 실패는 회복되지 않는다

GR00T N1.7 파인튜닝 모델에 대해 diffusion denoising step을 K=4(기본) → K=8로 늘려 재평가했으나
0/10 → 0/10으로 변화가 없었다. 추론 시 샘플링 예산의 문제가 아니라 정책 자체의 문제임을 보여준다.
